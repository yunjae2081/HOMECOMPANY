...
$(document).on("click","a", function() {
  if (this.id == '1') { // 인기순의 버튼을 눌렀음
    ...
    $.ajax({
      url: "${pageContext.request.contextPath}/side/bestSearch.json",
      type: "POST",
      datatype: "JSON",
    ...
    l_scroll_block = 0; // 최신순의 버튼을 눌렀으니 무한 스크롤 방지 해제
    $.ajax({
      url: "${pageContext.request.contextPath}/side/latestSearch.json",
      type: "POST",
      datatype: "JSON",
      data: {category:category},
      success: function (data, status) {
  ...
  $(document).scroll(function() {
    if(b_scroll_block == 0 && l_scroll_block == 0){ // 모든 상황에서 무한 스크롤 방지 해제
		...
		if(scrollHeight >= documentHeight){
		  if(flag == 0){
		    $.ajax({
		      url: "${pageContext.request.contextPath}/side/bestSearchExtend.json",
		      type: "POST",
		      datatype: "JSON",
		    ...
		    $.ajax({
		      url: "${pageContext.request.contextPath}/side/latestSearchExtend.json",
		      type: "POST",
		      datatype: "JSON",
